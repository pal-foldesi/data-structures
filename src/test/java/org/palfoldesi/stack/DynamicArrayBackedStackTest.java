...
public class DynamicArrayBackedStackTest {
    @DisplayName("Is initially empty")
    @Test
    public void givenEmptyStack_whenIsEmptyCalled_thenTrueIsReturned() {
        Stack<String> stack = new DynamicArrayBackedStack<>(String.class);
    ...
    public void givenNotEmptyStack_whenPopping_thenCorrectItemIsReturned() {
        int itemCount = 100;
        Stack<Integer> stack = new DynamicArrayBackedStack<>(Integer.class);
        for (int i = 0; i < itemCount; i++) {
            stack.push(i);
        }
